...
try:
    fit = RandomForestClassifier().fit(train_features,train['airline_sentiment'])
    pred = fit.predict(test_features)
except Exception:
    fit = RandomForestClassifier().fit(dense_features,train['airline_sentiment'])
    pred = fit.predict(dense_test)
accuracy = accuracy_score(pred,test['airline_sentiment'])
print('Accuracy of RandomForestClassifier is '+ str(accuracy))